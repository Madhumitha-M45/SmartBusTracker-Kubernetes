...
public class ETANotificationService {
    ...
    public void checkAndSendNotificationForSubscriber(ETAResponse response, String deviceToken) {
        ...
        int currentEta = extractMinutes(response.getBoardingEta());
        ...
        if (previousEta == null) {
            ...
            System.out.println("[ETA-NOTIFICATION] First ETA recorded for Bus " + response.getBusId() + ": " + currentEta + " mins");
        ...
        Integer milestone = findCrossedMilestone(previousEta, currentEta);
    ...
    private String buildMessage(ETAResponse response, int etaMinutes) {
        ...
        String stop = (response.getBoardingStop() != null) ? response.getBoardingStop() : "your boarding stop";
    ...
    private int extractMinutes(String eta) {
        ...
        String text = eta.trim().toLowerCase();

        if (text.contains("arrived") || text.contains("reached")) {
        ...
        if (text.contains("hr") || text.contains("hour")) {
            ...
            String[] parts = text.split("hr|hour");

            String hoursOnly = parts[0].replaceAll("[^0-9]", "");

            if (!hoursOnly.isEmpty()) {

                totalMinutes += Integer.parseInt(hoursOnly) * 60;

            }

            if (parts.length > 1) {

                String minsOnly = parts[1].replaceAll("[^0-9]", "");

                if (!minsOnly.isEmpty()) {

                    totalMinutes += Integer.parseInt(minsOnly);
